...
import {
  ...
  WorkScheduleApi,
  WorkScheduleDto,
  WorkScheduleFinishDeathReportParam,
...
const WorkScheduleFinishDeathReportForm = (props: {
  ...
  const saveHook = useOpenApiFpRequest(
    WorkScheduleApi,
    WorkScheduleApi.prototype.workScheduleFinishDeathReportPost
  );
  useEffect(() => {
  ...
  return (
    <div>
      <Form<WorkScheduleFinishDeathReportParam>
        form={form}
        onFinish={async (data) => {
          await ConfirmAsync({
            title: "确认",
      ...
      >
        ...
        <Form.Item label="死亡数" name="count">
          <InputNumber min={0} />
        </Form.Item>
        {/*<Form.Item label="所在苗床" name="pondId">*/}
        {/*  <Selector type={PondType.苗床} />*/}
        {/*</Form.Item>*/}
        <Form.Item label="记录时间" name="finishTime">
          <DatePicker showTime />
        </Form.Item>
...
export default WorkScheduleFinishDeathReportForm;
